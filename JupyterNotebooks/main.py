...
dimtext = """\
# Dimension-Reduction
Dimension-reduction transformations are used to condense high dimensional data down to a smaller number of dimensions.
We wish to leave only the dimensions which provide the most amount of information. 

## Principal Component Analysis
The principal component analysis (PCA) is one of the methods for dimension-reduction. It performs a change of basis and the new basis is chosen, such that the i-th principal component is orthogonal to the first i-1 principal components and the direction maximizes the variance of the projected data.
Instead of considering all the dimensions, we pick the necessary number of principal components. We use the Python library sklearn for both PCA.
